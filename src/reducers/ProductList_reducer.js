...
const productlistReducer = (state=initialState, action) => {
    ...
    switch (action.type) {
        case LOAD_ITEMS:
            ...
            if (action.payload.manager) {
                Object.keys(items.manager).forEach((item)=>{
                    ...
                    if (Object.keys(items.manager[item].filters).length===4
                      &&items.manager[item].filters.itemType==='earrings') {
                      items.manager[item].complited = true;
                    }
                });
            }
        ...
        case UPDATE_ITEM:
            ...
            items.manager[item].filters = Object.assign(
              items.manager[item].filters,
